...
def get_gaze(eyedata, eventcode, behavior, start, stop,
    ...
    fixation_list = []
    saccade_list = []
    start_code, stop_code = EVENT_CODES[start], EVENT_CODES[stop],
    ...
    for trial in behavior.Trial.unique():
        try:
            ...
            result = NH.detect_saccades_NH(x, y, freq=freq)

            trial_fixations = result.fixations.query('(t_off>@epoch_on) & (t_on<@epoch_off)').copy()
            trial_saccades = result.saccades.query('(t_off>@epoch_on) & (t_on<@epoch_off)').copy()

            trial_fixations[start] = epoch_on
            trial_fixations[stop] = epoch_off
            trial_fixations.insert(0, 'Trial', trial)

            trial_saccades[start] = epoch_on
            trial_saccades[stop] = epoch_off
            trial_saccades.insert(0, 'Trial', trial)

            fixation_list.append(trial_fixations)
            saccade_list.append(trial_saccades)

        except Exception as e:
            print(f'Trial {trial}: skipped ({type(e).__name__}: {e})')
    ...
    all_fixations = all_fixations[
        (all_fixations.x_mean.abs() < xy_bound) & (all_fixations.y_mean.abs() < xy_bound)
...
def plot_saccades(saccades, edge=15, alpha=0.1):
